...
function scanSkills(directory: string, isArchived: boolean) {
    console.log(`Scanning skills in: ${directory} (Archived: ${isArchived})`);

    if (!fs.existsSync(directory)) {
        if (!isArchived) { // Only warn for missing main directory
    ...
    const skillDirs = fs.readdirSync(directory).filter(d =>
        fs.statSync(path.join(directory, d)).isDirectory()
    );

    for (const skillId of skillDirs) {
        const skillFile = path.join(directory, skillId, 'skill.md');
...
scanSkills(SKILLS_DIR, false);
const ARCHIVE_SKILLS_DIR = path.join(PROJECT_ROOT, 'ARCHIVE', 'skills');
scanSkills(ARCHIVE_SKILLS_DIR, true);
